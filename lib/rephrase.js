...
function astTransformAST(tree, defineTree) {
    var blockComments = _.filter(defineTree.comments, function (c) {
        return c.type == 'Block';
    });
    var forallRe = /forall((\s+[a-z]+)*)\s*\./;
    var rules = _.map(
        _.filter(
    ...
    return _.reduce(rules, rephrase, tree);
}
function astTransformSource(source, defineSource) {
    var tree = esprima.parse(source, {comment: true});
    var defineTree = esprima.parse(defineSource, {comment: true});
    return astTransformAST(tree, defineTree);
}
rephrase.transformSource = function (source, defineSource) {
    return escodegen.generate(astTransformSource(source, defineSource));
};
rephrase.astTransformSource = astTransformSource;
rephrase.astTransformAST = astTransformAST;
